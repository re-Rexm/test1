...
document.addEventListener("DOMContentLoaded", function () {
  ...
  // Wait for camera to get user's GPS position
  scene.addEventListener('loaded', function () {
    const camera = document.querySelector('a-camera');
    if (!camera) return;

    // Get user's current position from gps-camera
    const userPos = camera.getAttribute('gps-camera');
    if (!userPos) {
      console.error('Could not get user GPS position');
      return;
    }

    const baseLat = userPos.latitude;
    const baseLng = userPos.longitude;
    const offset = 0.0002; // ~20 meters offset
  ...
  function toggleEventDisplay(eventId) {
    ...
    if (isMarkerVisible) {
      // Show text (keep at GPS location)
